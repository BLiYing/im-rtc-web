...
- **一律先在 worktree 上改，改完再合回 main。不许直接在 main 的工作区改代码。**
  ...
  **合完要在 main 上再跑一次 `./scripts/test.sh`**——两个各自都绿的分支合到一起可以是红的，
  ...
  > 测试代码那份按「主检出 / `.claude/worktrees/<分支>`」两种布局算出兄弟仓**唯一**该在的位置，
  > 于是不走脚本直接 `npx vitest` / `swift test` 也能工作。
  > **别写成「往上逐级找到根」**（web 最初就这么写的，2026-09-10 code-review 抓到）：
  > 同级缺失时它会爬出本仓、捡到上层某份旧的 im-rtc-server，拿旧向量跑绿——正是「找不到就抛错」要防的事。
