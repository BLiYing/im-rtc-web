...
#
# 阈值 600（2026-09-08 由 400 上调，见下）。超标的正确处理仍然是**拆分**，不是再抬阈值：
#   ① 组件膨胀     → 抽子组件 + 抽自定义 hook（useCallControls），逻辑与渲染分离。
#   ② 纯计算逻辑   → 抽独立 .ts 模块并直接单测（不经 React）。
#   ③ 状态机膨胀   → 按状态族拆文件。
# 姊妹项目 im-web 的教训：App.tsx 长到 4891 行后拆了很久才收口。别重蹈覆辙。
#
# 为什么从 400 抬到 600：**这是按语言校准，不是给某个文件开口子。**
#   · 五端同一套协议、同一批能力，其余四端的阈值是 Android/iOS/桌面 600、Go 1200；
#     TS 的 400 是六个仓里最严的一档，而 TS 并不比 Swift 更紧凑。
#   · 本仓有一类文件天生贴线：`engine.ts` 是**公开门面**，它必须枚举整套公开 API
#     （25 个方法）连同给宿主看的文档。它曾经正好 400/400，加一行就红。
#     2026-09-08 先按规矩拆了一刀（接线与媒体编排各自成模块，见 engineWiring.ts /
#     media/engineMediaApi.ts），降到 353——**拆分在前，抬阈值在后**，顺序不能倒过来。
#     再往下拆就只剩把公开签名与文档挪走，那是拿可读性换行数。
#   · 400 那一档并非白设：它挡住的是 React 组件那类真会失控的膨胀，这条约束不因阈值
#     上调而作废——组件层面另有「单个函数 ~50 行 / 单个组件 ~120 行」那条更紧的尺子。
#
