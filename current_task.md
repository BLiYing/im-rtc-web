...
**2026-09-11 下午：群通话真机报的五个问题逐个修，直接在 main 上改（未提交）。本仓只有一个（3b）。**

**通话中点开摄像头，别人看得见我、我自己那格没画面**（真机 09-11 07:49 起，偶发）：
`useCallActions.toggleCamera` 通话中发布摄像头之后漏了 `dispatch({ type: 'localCamera' })`，本端格子挂不上轨道。
以前被「权限门顺手起的预览」遮住了——那条路会先把 `localCameraCid` 填上；群通话改成默认关摄像头进来之后，
通话中点开必走漏掉的这条。「偶发」是因为进房前起过预览（1v1 视频默认开）就不会踩到。

**Web 九宫格按源的实际长宽显示是设计要求**，iOS / Android 正方形填满那一刀不动本仓。

`./scripts/test.sh` 全绿（13 步），uikit 118 条（新增「通话中打开摄像头」1 条）。**没在浏览器里跑过**。

上一刀（接听时摄像头权限、群通话默认关摄像头，§01 + §11-10）已合 main，细节看 `git log`。
...
1. **群视频（默认关摄像头）进房后点开摄像头**：自己那格立刻有画面、对端也看得到；关掉再开仍然有。**先重起 vite。**
2. 上一刀没验的：群视频来电横幅最左摄像头按钮关态；1v1 视频来电关摄像头再接只要麦克风。
