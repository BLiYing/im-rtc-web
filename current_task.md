...
**2026-09-11：接听时的摄像头权限、群通话默认关摄像头，三端一起对齐交互稿 `RTC_CALL_UX_FLOWS.html` §01 权限时机表 + §11-10。已合 main，用户自己打包真机验。**

本仓三处：

1. **群视频默认关摄像头**：`callView.ts` 的 `defaultCameraOn(mediaType, isGroup)` = 视频且非群（会议照旧默认开）。
   来电横幅最左那颗摄像头按钮是关态；发起群视频也不再一上来就开摄像头。
2. **接听申请哪些设备**走 `devicesForAnswering(mediaType, cameraOptedOut)`（与 iOS / Android 同一条判据）：
   视频且没在来电页亲手关摄像头 → 麦克风 + 摄像头。`cameraOptedOut` 只在 incoming 阶段切摄像头时置位，群通话默认关不算。
3. **探摄像头权限不再开着摄像头**：engine 加 `probeCamera()`（拿一次流立刻停掉，每个 adapter 只探一次），
   `usePermissionGate` 用它探；本地预览只在摄像头开着时起。

`./scripts/test.sh` 全绿（13 步）。**这一刀没在浏览器里跑过**。
...
1. **群视频来电**：横幅最左摄像头按钮关态；接听弹麦克风 + 摄像头；进房自己那格是头像，点开后对端看得到。
2. **群视频发起**：摄像头默认关，对端看到头像。
3. **1v1 视频来电关摄像头再接**（站点设置里先把摄像头设成阻止）：只要麦克风、能接通；接通后点开摄像头 → 「无权限」、通话不断。
