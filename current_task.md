...
| 包 | 内容 | 怎么验的 |
|---|---|---|
| `@im-rtc/call-engine` | 协议层、WS 客户端、通话/房间/总状态机、WebRTC 适配器 | 172 个用例，含 50 条四仓共用向量 |
...
**本仓剩下的**
- 预警线（320 行 / 上限 400）上的四个文件：`engine.ts` 376、`signaling/connection.ts` 382、
  `state/roomMachine.ts` 339、`call-uikit-react/src/state/callView.ts` 328。
  **下一次动它们时先拆**。（`engine.ts` 这轮已经拆出 `media/mediaPlane.ts`——
  候选进出、远端轨道落地、PC 状态转内部事件——才回到 400 以内。）
...
- **刷新丢失进行中状态**：MediaStream 不能跨刷新持久化；通话中刷新即掉线，
  UI 要正确表现（重连而非假装还在）。
- **Demo 不会自动换 token**：服务端重启后 HS256 密钥变了，旧页面带着过期 token 重连
  三次后 engine 会抛 `onKickedOut` 收手（协议 §1.5）。协议里 4401 就是「换个 token 再来」，
  **换 token 是宿主的事**，engine 不该自己去要——但 Demo 该演示这一步，暂未做，
  现在的表现是「被踢回登录态、要手动重登」。
- **会议房空了就销毁**：最后一个人离开后房间即关（服务端日志「房间已空，已关闭」），
  所以 Demo 里刚离开的那个房间号再点「进会议」会得到 404。这是对的，
  但 Demo 把刚离开的房间号留在了输入框里，容易误会——待改。
