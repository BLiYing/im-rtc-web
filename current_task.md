...
**Web 看对端重开摄像头「刷新一闪」——第二版已改、未提交，待浏览器复测**（2026-09-11 20:03 web bob 看 iOS carol；Android 同一问题 20:04 真机已验好）。
...
- 用户复测「Web 看 iOS 频繁开关摄像头」并报时间；读 `wait_ms` 分布（应落在 200ms 以上）、`skipped`（≥1 说明挡掉了旧帧）、`judged_by`（Chrome 应为 `receive_time`）；
  常撞 2 秒兜底就再查（后台标签页 / 对端迟迟不出关键帧）；`skipped=0` 仍闪则旧帧假设不成立，另查。
